(ns drae.doc-test
  ...
  (:require [clojure.test :refer :all]
            ...
            [drae.j.VisualElement :refer [items]]
            [drae.doc :refer :all]
            )
  )
...
(deftest test-string->boolean
  (testing "Test the string->boolean routine."
    (let [string->boolean (private-function string->boolean drae.doc)]
...
(deftest test-common-aspect
  (testing "Test the common-aspect function."
      (let [common-aspect (private-function common-aspect drae.doc)]
...
(deftest test-vimage
  (testing "Test creation of VImage instances. NOTE: If failing on instance? test, try cleaning and reloading."
    (let [x (make-vimage "image-path/image.jpg" (make-bbox 0.0 0.0 10.0 10.0))
      ...
      (is (true? (instance? drae.j.VisualElement.VImage x)))
...
(deftest test-next-up
  (testing "Test the next-up function."
    (let [next-up (private-function next-up drae.doc)
...
(deftest test-get-vdocument
  (testing "Testing get-vdocument on :core-test 1 pdf."
    (let [v (get-vdocument (corpus :core-test 0))
          page? (partial instance? drae.j.VisualElement.VPage)
          vel? (partial instance? drae.j.VisualElement.El)
          vtext? (partial instance? drae.j.VisualElement.VText)
          vdoc? (partial instance? drae.j.VisualElement.VDocument)]
