(ns drae.corpus
  "*Routines to access predefined corpora documents. Corpora are defined in the 
    config files `drae-settings.edn` and `drae-default-settings.edn`.*"
  (:import (java.io File))
  (:require [clojure.java.io :refer [file as-url input-stream output-stream copy]]
            [clojure.pprint :refer [print-table]]
            [drae.util :refer [file-type? uerr]]
      ...
      (cond 
        ...
        :else 
         (if-let [corpus-desc (get cmap corpus-name)]
           ...
           (uerr "Cannot find corpus named %s in DRAE configuration files." corpus-name))
        )))
  ([corpus-name n] 
