...
;;;;
(ns drae.util
  "Utility routines used by DRAE. These routines are not part of the API, and may change, but
...
(defn cache-directory 
  "The DRAE cache directory for a given file. Always called .drae-cache in the 
   same directory as the file itself. If cache directory does not exist, creates
   it."
  [f]
  (let [cache-dir (file (str (.getParent f) "/.drae-cache"))]
    (make-dirs cache-dir)
    (swap! current-cache-dirs conj cache-dir) ;; Add to list of cache directories.
    (.deleteOnExit cache-dir) ;; When JVM exits, remove drae-cache.
    cache-dir))
