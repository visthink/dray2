(ns drae.doc
  ...
  (:import (java.awt.geom Rectangle2D)
           (drae.j BoundingBox)
           (drae.j.VisualElement VDocument VText VPage)
           )
  (:require [clojure.string :as s]
            [clojure.data.xml :refer [parse-str]]
            [clojure.zip :refer [xml-zip up down right node end? edit root]]
            [drae.util :refer [resolve-cache-file-for cache-directory pairwise-replace 
                               decode-url pairwise-partition-when uerr]]
            [clojure.java.io :refer [file]]
            [drae.util.math :refer [abs-diff]]
            [drae.xml :refer [cljxml->prxml prxml-tag prxml-attrs prxml-content]]
            [drae.ext.pdftoxml :refer [pdftoxml-output]]
...
(defn make-bbox 
  ...
  ([x y width height]
    (let [->dbl #(if (string? %) (Double/parseDouble %) (double %))] ; Needed to handle doubles, ints *and* strings.
      (drae.j.BoundingBox. (->dbl x) (->dbl y) (->dbl width) (->dbl height))))
...
(defn texts-adjacent?
  ...
  [txt1 txt2]
  (and (instance? drae.j.VisualElement.VText txt1)
       (instance? drae.j.VisualElement.VText txt2)
       (valigned? txt1 txt2)
...
(defmethod merge-adjacent-vtexts drae.j.VisualElement.VPage [page]
  (merge-adjacent-vtexts-in-page page))
;  (make-vpage (.number page) (.bbox page) (merge-adjacent-vtexts (.getItems page))))

(defmethod merge-adjacent-vtexts drae.j.VisualElement.VDocument [vdoc]
...
(defn ^drae.j.VisualElement.VDocument get-vdocument
