...
public class WorkingSet {
	...
	/**
	 * Creates a new working set and adds the items inside of rect from the
	...
	/*
	 * addTag - Add a single tag to this working set. Takes the first token of
	 * the string and lowercases it to create canonical tag.
	 * 
	 * @param tag The token as a String.
	 */
	...
	public List<Rectangle> groupHorizUniqueSections(List<Rectangle> inputRectangleList) {
		...
		while (consecutiveRejections < rectangleList.size()) {
			...
			for (Rectangle candidate : columnCandidates) {
				...
				// Rectangle combinedRectangleInner = new
	...
	public Rectangle determineHeaderRow(List<Rectangle> columns) {
		...
		// second pass -- go through again and pull out everything above the
		// line
	...
	public void normalize() {
		...
		while (shuffled) {
			...
			for (WorkingSet child : sortedByArea) {
				...
				// 2nd, if any sibling working sets are entirely within this
				// child,
					...
					if (sibling != child) {
						...
						if (child.getBbox().contains(sibling.getBbox())) {
							...
							if (!child.containsChild(child)) {
								// System.out.println("moving "+sibling +" into
								// "+child);
