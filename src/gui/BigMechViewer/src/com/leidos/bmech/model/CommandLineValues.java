...
/**
 * Command-line handler for DRAY. Called at beginning of DRAY run when called
...
public class CommandLineValues {
	...
	@Option(name = "-b",
			// aliases = { "--batch", "--nogui" },
			required = false, usage = "Run in batch mode. Do not start graphical interface.")
	private boolean	batch;

	// Output JSON representation.
	@Option(name = "-o",
			// aliases = { "--out" },
			required = false, metaVar = "JSON-filename", usage = "JSON output filename or path (optional).")
	private String		out					= "DEFAULT";

	// Help.
	@Option(name = "-h", aliases = { "-?", "-help" }, required = false, usage = "Print this usage message and exit.")
	private boolean	help;

	// Skip overlays
	@Option(name = "-x",
			// aliases = {"--skipoverlays"},
			required = false, usage = "Skip loading working set overlays, even if available.")
	...
	@Option(name = "-t", required = false, usage = "Run the table detection code on all table working sets in the loaded file.")
	private boolean	useTableCode;

	// Single argument - PDF file or directory with PDF files.
	@Argument(index = 0, required = false, metaVar = "source", usage = "PDF file (or directory containing PDF files).")
	String				sourceName			= "";
	...
	public CommandLineValues(String... args) {
		...
		} else { // Should never reach here unless there's a problem with the
					// code.
