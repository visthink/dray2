...
public class DataManager extends Observable {
    ...
    EvidenceGatherer eg;
    int preprocessState; //-1: not started; 0: underway; 1: done
	...
	/**
	 * initialize DataManager with data from pdf. internal vdocument will be loaded by
	 * the constructor.
	 * @param pdf
	 * @param lazyLoadImages false if should load images, true if should not load images
	 */
	...
	/**
	 * load the page images from pdf and load the vdocument data
	 * from a closure call.
	 * param lazyLoadImages -- set to true if images should only
	...
	public LayerList getLayerList() {
		if(view.getCurrentWS() == null) return null;
	...
	/**
	 * called from the GUI, sets the File to the pdf and
	 * calls the loadFromPdf() to get the data.
	 * @param pdfFile
	 */
...
**/
	...
	/**
	 * Get a reference to the drae.j.VDocument instance currently being
	 * used by the GUI
	...
	/**
	 * Return a list of working sets that contain the given x,y 
	 * Useful for detecting mouse events
	 * @param x
	 * @param y
	...
	/**
	 * Return a list of working sets that have an edge at the given x,y 
	 * Useful for detecting resize events
	 * @param x
	 * @param y
	 * @return the list of working sets
	 */
	...
	/**
	 * get a list of drae.j.VisualElement.El objects that are FULLY
	 * inside of the rectangle
	 * @param dragRectDescaled the rectangle
	 * @return
	 * 
	 */
...
/*	
	public List<El> getSelected(){
		return selectedEls;
	}
*/
	/**
	 * creates a new working set from the selected Visual Elements
	...
	/**
	 * Delete the current working set. also sets the current working set
	 * to be the parent of the old working set
	 */
	...
	/**
	 * Deletes the given working set and all of its children from the 
	 * WorkingSet tree. If the current working set would be deleted, then
	 * the current working set switches to the parent of the deleted node.
	 * @param victim -- the working set to be deleted
	 */
	public void deleteWS(WorkingSet victim) {
		//check if null
		if(victim == null) return;
		//check if we're trying to delete a page
		if(victim.getParent() == headWS) return;
	...
	public BufferedImage getPageImage(int pageNumber){
		...
		if(pageImageStatus[pageNumber-1] == -1){
			...
			return defaultImage;
		} else return pageImageList[pageNumber-1];	
	}
	public BufferedImage getPageIcon(int pageNumber){
		...
		if(pageImageStatus[pageNumber-1] == -1){
			...
			return defaultImage;
		} else return pageIconList[pageNumber-1];	
	}
		...
		SwingWorker<BufferedImage, Void> worker = new SwingWorker<BufferedImage, Void>() {
			...
			protected BufferedImage doInBackground() throws Exception {
				if(Doc.useGhostscript()){
					...
					BufferedImage ret = new BufferedImage(transparentImg.getWidth(null), transparentImg.getHeight(null), BufferedImage.TYPE_INT_ARGB);
			...
			protected void done() {
				...
				try {
					image = get();
					double aspect = (double) image.getWidth()
							/ (double) image.getHeight();
					BufferedImage iconImage = new BufferedImage(128,
							(int) (128 / aspect), BufferedImage.TYPE_INT_RGB);
					...
					setChanged();//Observable Pattern: inform gui that images have changed
					notifyObservers();
