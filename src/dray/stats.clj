(ns dray.stats
  "Statistical routines for analyzing document structure. Not graphical -- see
   dray.charts for graphical display routines.
  ...
  (:require [dray.util :refer [uerr pairwise-group-by instances]]
            [dray.util.math :refer [abs-diff]]
            [dray.util.map :refer [select-keys-if merge-map-keys]]
            [dray.doc :refer [vel-x vel-y vel-height vel-width vel-max-x vel-center-x]]
            )
  )
...
(def texts-in "All VText instances in this collection." (partial instances dray.j.VisualElement.VText))
