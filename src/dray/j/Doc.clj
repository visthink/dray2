(ns dray.j.Doc
  ...
  (:require [clojure.java.io :refer [file]]
            [dray.ext.ghostscript :refer [use-ghostscript? generate-page-images page-image-file page-image-for]]
            [dray.doc :refer [get-vdocument]]
            )
  (:gen-class :name dray.j.Doc
              :state state
              :constructors {[] []}
              :prefix "-"
...
(defn -saveWStoOverlay 
  ...
  [dm json] 
  (if-let  [save-fn (find-var 'dray.manager/dm-save-ws-to-overlay)]
    (save-fn dm json)
...
(defn -restoreWSfromOverlay 
  ...
  [dm json-file]
  (if-let [restore-fn (find-var 'dray.manager/dm-restore-ws-from-overlay)]
