(ns dray.j.VisualElement
  ...
  (:require [clojure.string :as s]
            ...
            [dray.util :refer [pairwise-split-when uerr]]
            )
  )
...
;;; Bounding Boxes
;;; ------------------------------------------------------------------------------
(gen-class :name dray.j.BoundingBox
...
(defmethod print-method dray.j.BoundingBox [x writer]
...
(defn my-bbox-union 
  ...
  (let [u (reduce (fn [^Rectangle2D b1, ^Rectangle2D b2] (.createUnion b1 b2)) bboxes)]
    (dray.j.BoundingBox. (.getX u) (.getY u) (.getWidth u) (.getHeight u))))
...
(defmethod print-method dray.j.VisualElement.VDocument [x writer]
...
(deftype VPage [^Integer number, ^dray.j.BoundingBox bbox, ^:volatile-mutable ^java.util.List items]
  Object
...
;;; The next few functions are copied from the dray.doc code. We don't include it
...
(defrecord VText [^String text, ^dray.j.BoundingBox bbox, ^java.util.List items, ^TextStyle style]
  Object
  (toString [this] 
    (let [x (.x (.bbox this))
          y (.y (.bbox this))]
...
(defrecord VTextToken [^String text, ^dray.j.BoundingBox bbox, style]
  Object
   (toString [this]
...
(extend dray.j.BoundingBox    json/JSONWriter {:-write write-bbox-json})
