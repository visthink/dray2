(ns dray.textblock
  ...
  (:require [dray.util.math :refer [=* abs-diff]]
            [dray.util.map :refer [apply-fn-map]]
            [dray.util :refer [meta-seq pairwise-partition-when pairwise-replace]]
            [dray.doc :refer [vel-x vel-max-x vel-y vel-height make-vtext-block]]
            )
  )
