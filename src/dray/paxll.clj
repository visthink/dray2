(ns 
  ...
  (:import (java.net InetAddress)
           ...
           (dray.j.paxtools XPattern XMatch)
           )
  (:require [clojure.repl :refer :all]
            ...
            [dray.util :refer [uerr file?]]
            [dray.config :refer [dray-setting]]
            [dray.util.char :refer [char-replacer *greek-map*]]
            [dray.j.paxtools :refer :all]))
...
(defn- ontology-map "Map of available ontologies." []
  {:post [(map? %)]}
  (or (dray-setting :ontologies) 
      (uerr "Could not find :ontologies in DRAY setting files.")))
...
(defmethod describe dray.j.paxtools.XPattern [xp]
  (let [>> (fn [& x] (println (apply format x)))
      myname #(with-out-str (print-method % *out*))
    ;  constraints (.getConstraints p)
    ;  size (count constraints)
