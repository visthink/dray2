(ns dray.ext.ghostscript
  "Routine for calling Ghostscript to produce background image. Depracted -- will likely
   be removed in later version because DRAY will use Java-based rendering in the future."
  (:import (java.io File))
  (:require [clojure.java.shell :refer [sh with-sh-env with-sh-dir]]
            [dray.util :refer [cache-directory rootname uerr]]
            [dray.util.exec :refer [add-to-system-path unix-which *additional-exec-paths*]]
...
(defn ghostscript-path 
  "Attempts to find and return the location of the Ghostscript executable.
   See also [[dray.util.exec/*additional-exec-paths*]] and [[dray.config/*gs-args*]]."
  []
  (or (dray-setting :ghostscript-executable)
...
(defn use-ghostscript?
  ...
  []
  (dray-setting :use-ghostscript)) ; If false or missing, don't use.
...
(defn gs-page-output-filestring [pdf page-no]
  (let [root (dray.util/rootname pdf)]
