(ns dray.ext.bee
  ...
  (:require [clojure.java.shell :refer [sh with-sh-dir]]
            ...
            [dray.util :refer [resolve-cache-file-for rootname make-dirs uerr]]
            [dray.util.map :refer [mapify]]
            [dray.util.exec :refer [unix-which]]
            [dray.doc :refer [make-bbox]]
...
(defn- bee-exe
  ...
  []
  (let [setting (dray-setting :bee-executable)]
    (if-not (empty? setting)
      setting
      (let [search-res (unix-which "beelibtester")]
        (if (empty? search-res) ;; Didn't find.
          (uerr "Could not find the executable path for bee. Set in dray-settings.edn or add to path.")
          search-res)))))
