(ns dray.gui 
  "Routines for accessing DRAY GUI elements from Clojure.The main GUI routines 
   ...
   This package also includes the routines for several default DRAY toys."
  ...
  (:require [clojure.java.io :refer [file]]
            ...
            [dray.util :refer [cache-directory]]
            [dray.util.inspect :refer [inspect]]
            [dray.wset :refer [ws-images ws-pdf]]
            [dray.j.Toys :refer [add-ws-toy add-sel-toy]]
            [dray.charts :refer [vels->dataset wset->dataset
                                 text-line-edge-scatter 
                                 text-histogram]]
            [dray.ext.bee :refer [bee-rep-for]]
            [dray.paxll :refer [pc2query model! model]]
            [dray.vtable :refer [ws-vtables]]
            ))
...
(defn gui-selected-items 
  "Current set of selected items in the DRAY GUI."
  ([gui] (.getSelected (.getView gui)))
  ([] (gui-selected-items (current-gui))))

(defn gui-working-set
  "The current working set in the DRAY GUI."
...
(defn toy-show-image "Show the first image in the working set in a frame." [ws]
  (let [vimage (first (ws-images ws))
        dray-cache (cache-directory (ws-pdf ws))
        image-file (file dray-cache (.bitmap-path vimage))
        bbox (.bbox vimage)]
    (-> (frame 
...
(defn run-gui "Run the DRAY GUI." 
  ([argstrings]
    (let #_[gui (ViewerApp.)] 
      [gui (ViewerApp/startDray (into-array String argstrings))]
